...
Shader "Skybox/ProceduralCloud" {
		...
		Properties{
			...
			_CloudCull("Cloud cull",range(0.8, 1.2)) = 1
			...
			_CloudNumber("Cloud number",range(0.01,0.3)) = 0.03
			...
			//              for(int i=0; i<int(kSamples); i++)
					...
					{
						float f = 0;
						f = 0.5000 * nnoise(x); x = x * 2.01;
						f += 0.2500 * nnoise(x); x = x * 2.02;
						f += 0.150 * nnoise(x); x = x * 2.03;
						f += 0.0625 * nnoise(x); x = x * 2.01;
						f += 0.0375 * nnoise(x);
					...
					float FBM(float3 p)
					{
						...
						return _CloudDensity * (noise(p) * 2 - _CloudCull);
					}
					...
					float rayPlaneIntersect(float3 s0_r0, float3 rd, float sr)
					{
						if (rd.y > 0.0)
						{
							return -(sr + s0_r0.y) / rd.y;
						}
					...
					half4 frag(v2f IN) : SV_Target
					{
						...
						// if y < 0 [eyeRay.y > 0] - sky
							...
							c.a *= smoothstep(0.1, 0.5, 1.0 - yy);

							col = lerp(col, c.rgb * lerp(col.rgb, _CloudColor.rgb, _CloudColor.a), saturate(c.a * _CloudColor.a));
#endif
