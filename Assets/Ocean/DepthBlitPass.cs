...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    {
       ProfilingSampler m_ProfilingSampler = new ProfilingSampler("DepthBlitPass");
        ...
        public DepthBlitPass(RenderPassEvent evt, Material copyDepthMaterial, bool shouldClear = false, bool copyToDepth = false, bool copyResolvedDepth = false)
        {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            ExecutePass(context, ref cmd, ref renderingData.cameraData, source, destination);
            context.ExecuteCommandBuffer(cmd);
            cmd.Clear();
            CommandBufferPool.Release(cmd);
        }

        private void ExecutePass(ScriptableRenderContext context, ref CommandBuffer cmd, ref CameraData cameraData, RTHandle source, RTHandle destination)
        {
            var copyDepthMaterial = m_PassData.copyDepthMaterial;
            var msaaSamples = m_PassData.msaaSamples;
            var copyResolvedDepth = m_PassData.copyResolvedDepth;
            var copyToDepth = m_PassData.copyToDepth;
